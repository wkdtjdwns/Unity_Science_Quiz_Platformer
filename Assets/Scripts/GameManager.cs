...
public class GameManager : MonoBehaviour
{
    ...
    void Start() // Awake() 함수가 실행되고 난 후
    {
        ChangeBgm(0); // 배경음을 초기화 시킴
    }
    ...
    public void Option() // 옵션창 띄우는 함수
    {
        ...
        // 옵션창의 유무에 따라서 옵션창을 띄우거나 내림
        option_group.gameObject.SetActive(is_option);

        // 옵션창이 띄워져 있을 땐 시간을 멈추고 아니면 시간을 흐르게 함
    ...
    {
        SoundManager.instance.PlaySound("Button"); // 버튼을 누르는 소리를 출력함

        StopAllCoroutines(); // 모든 코루틴 함수를 멈춤

        is_quiz = true; // 퀴즈를 내는 중이라고 알려줌
    ...
    public void CheckAnswer(int ans_btn) // 누른 버튼의 int 값을 받아옴
    {
        switch (ans_btn) // 누른 버튼의 int 값에 따라서
        {
            case 0: // 0이면 O 버튼
        ...
        if (is_correct)
        {
            ...
            if (player_logic.life < 3) // 생명이 3보다 적으면
            {
                // 생명을 추가하고 생명 UI를 업데이트하는 함수 실행
                player_logic.life += 1;
